...
export class CreditNoteComponent {
  ...
  constructor() {
    ...
    effect(() => {
      this.invoiceService.getInvoiceById(+this.invoice_id()!).pipe(
        tap(data => {
  ...
  addToDisabledList(product_id: number) {
    ...
    if(idFinded) {
      const filteredList = this.disabledList().filter(id => id !== product_id)
  ...
  removePrice(product_id: number) {
    const product = this.invoice()?.products.find((value) => value.id === product_id)
