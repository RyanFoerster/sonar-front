...
export class DiaryComponent {
  ...
  protected selectedFilter = signal<'upcoming' | 'past' | 'hidden' | 'all'>(
    'all'
  );
  protected userInvitedFilter = signal<
    'invited' | 'accepted' | 'refused' | 'all'
  >('all');
        ...
        if (this.typeOfProjet() === 'PRINCIPAL') {
          ...
          this.groupAccountService
            ...
            .pipe(
              ...
              switchMap(() => {
                return this.eventService.getEventsByGroupId(
                ...
                for (const event of data) {
                  if (
                    event.participants?.find(
                      (user) => user.id === this.connectedUser()?.id
                    )
                  ) {
              ...
              tap((data: EventEntity[]) => {
                ...
                const findedUser = this.selectedEvent()?.user_status?.find(
                  (user) => user.user_id === this.connectedUser()?.id
                );
                if (findedUser) {
                  this.isUserAccepted.set(findedUser.status === 'accepted');
                }
                this.eventOrganisateur.update((value) => [
                  ...value,
                  ...this.selectedEvent()?.organisateurs!,
                ]);
              })
            )
            .subscribe();
  ...
  confirmEvent() {
    this.eventService
      .confirm(this.selectedEvent()!.id, this.selectedEvent()!)
      .subscribe(() => {
  ...
  cancelEvent() {
    ...
    this.eventService
      .cancel(this.selectedEvent()!.id, this.reason())
      .subscribe(() => {
  ...
  hideEvent() {
    ...
    this.eventService
      .hide(this.selectedEvent()!.id, this.reason())
      .subscribe(() => {
  ...
  userConfirmEvent(user: UserEntity) {
    this.eventService
      .userStatus(this.selectedEvent()!.id, user.id, 'accepted')
      .pipe(
        tap(() => {
  ...
  userRefuseEvent(user: UserEntity) {
    this.eventService
      .userStatus(this.selectedEvent()!.id, user.id, 'refused')
      .pipe(
        tap(() => {
          if (user.id === this.connectedUser()?.id) {
  ...
  createEvent(ctx: any) {
    if (this.createEventForm.valid) {
      ...
      this.eventService
        .create(this.createEventForm.value, this.accountGroup()!.id)
        .subscribe((event) => {
      ...
      if (this.isSameDate()) {
        ...
        if (
          !this.duplicateEventForm.value.start_time ||
          !this.duplicateEventForm.value.end_time ||
          !this.duplicateEventForm.value.rendez_vous_date
        ) {
        ...
        eventDto.rendez_vous_date =
          this.duplicateEventForm.value.rendez_vous_date;
      }

      if (this.isSameParticipants()) {
        eventDto.organisateurs_ids = this.selectedEvent()?.organisateurs!.map(
          (organisateur) => organisateur.id
        );
        eventDto.invitations_ids = this.selectedEvent()?.invitation!.map(
          (invitation) => invitation.user.id
        );
      } else {
        eventDto.organisateurs_ids =
          this.duplicateEventForm.value.organisateurs_ids;
        eventDto.invitations_ids =
          this.duplicateEventForm.value.invitations_ids;
      }

      this.eventService
        .create(eventDto, this.accountGroup()!.id)
        .subscribe((event) => {
          if (
            event.organisateurs.find(
              (user) => user.id === this.connectedUser()?.id
  ...
  checkUserOrganisateur() {
    return this.eventOrganisateur().find(
      (user) => user.email === this.connectedUser()?.email
    );
  }
  ...
  setInvitation(user: UserEntity) {
    if (this.userSelectedToBeInvited().includes(user)) {
      this.userSelectedToBeInvited.set(
        this.userSelectedToBeInvited().filter(
          (userInArray) => userInArray.id !== user.id
        )
      );
    } else {
      this.userSelectedToBeInvited.set([
        ...this.userSelectedToBeInvited(),
        user,
      ]);
    }
  }
  ...
  createInvitation(ctx: any) {
    ...
    for (const user of this.userSelectedToBeInvited()) {
      this.eventService
        .invite({ eventId: this.selectedEvent()!.id, userId: user.id })
        .pipe(take(1))
        .subscribe((data) => {
          this.userSelectedToBeInvited.set([]);
  ...
  protected isUserInvolved(user: UserEntity): boolean {
    ...
    const isOrganisateur = this.eventOrganisateur().some(
      (eu) => eu.id === user.id
    );
    const isParticipant =
      event.participants?.some((eu) => eu.id === user.id) || false;
    const isInvited =
      event.invitation?.some((eu) => eu.id === user.id) || false;
  ...
  updateEvent(ctx: any) {
    if (this.updateEventForm.valid) {
      ...
      this.eventService
        .update(
          this.selectedEvent()!.id as number,
          this.updateEventForm.value as EventDto
        )
        .subscribe((event) => {
  ...
  isUserAcceptedEvent(participant: UserEntity) {
    return (
      this.selectedEvent()?.user_status?.find(
  ...
  filterUserInvited(filter: 'invited' | 'accepted' | 'refused') {
    ...
    if (this.userInvitedFilter() === 'invited') {
      ...
      this.filteredUserInvited.set(
        invitations!
          .filter(
            (invitation) => invitation.status === this.userInvitedFilter()
          )
          .map((invitation) => invitation.user)
      );
    } else if (this.userInvitedFilter() === 'accepted') {
      const acceptedParticipants = this.selectedEvent()?.participants!.filter(
        (participant) => this.isUserAcceptedEvent(participant)
      );
      const acceptedOrganisateurs = this.selectedEvent()?.organisateurs!.filter(
        (organisateur) => this.isUserAcceptedEvent(organisateur)
      );
      this.filteredUserInvited.set([
        ...acceptedParticipants!,
        ...acceptedOrganisateurs!,
      ]);
    } else if (this.userInvitedFilter() === 'refused') {
      const refusedParticipants = this.selectedEvent()?.participants!.filter(
        (participant) => this.isUserRefusedEvent(participant)
      );
      const refusedOrganisateurs = this.selectedEvent()?.organisateurs!.filter(
        (organisateur) => this.isUserRefusedEvent(organisateur)
      );
      this.filteredUserInvited.set([
        ...refusedParticipants!,
        ...refusedOrganisateurs!,
      ]);
    }
  }
